package com.sulake.core.runtime.events
{
   ...
   public class EventDispatcher implements IEventDispatcher, IDisposable
   {
      ...
      private static const const_1110:uint = 1;
      
      private static const const_504:uint = 0;
      
      private static const const_783:uint = 2;
       
      
      private var var_745:Error;
      ...
      public function dispatchEvent(param1:Event) : Boolean
      {
         if(!_disposed)
         {
            ...
            if(_result == const_783)
            {
               if(var_745 != null)
               {
                  Core.crash("Error catched in EventDispatcher: undefined",var_745.errorID,var_745);
               }
               else
               {
                  Core.crash("Error catched in EventDispatcher. No error data available!",0,var_745);
               }
            }
            return _result == const_504;
         }
         return false;
      }
      ...
      private function eventProcessor(param1:Event) : void
      {
         ...
         var array:Array = var_280[event.type];
         if(array)
         {
            callbacks = [];
         ...
         _result = !!event.isDefaultPrevented() ? uint(const_1110) : uint(const_504);
      }
      ...
      public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            ...
            {
               _loc2_ = var_280[_loc1_] as Array;
