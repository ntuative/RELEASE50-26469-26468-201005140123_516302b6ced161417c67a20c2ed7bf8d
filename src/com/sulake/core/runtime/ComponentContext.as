   ...
   public class ComponentContext extends Component implements IContext
   {
      ...
      protected final function announceInterfaceAvailability(param1:IID, param2:Component) : void
      {
         ...
         while(_loc6_ < _loc5_)
         {
            ...
            if(_loc4_ == null)
            {
               error("Interface " + param1 + " still unavailabe!",true,Core.const_1149);
            }
      ...
      public final function loadFromFile(param1:URLRequest, param2:LoaderContext) : LibraryLoader
      {
         ...
         {
            if(var_248[_loc3_].url == param1.url)
            {
               return var_248[_loc3_].loadFromFile(param1);
            }
            _loc3_++;
         }
         var _loc4_:LibraryLoader = new LibraryLoader(var_1394);
      ...
      override public function toXMLString(param1:uint = 0) : String
      {
         ...
         var _loc11_:int = 0;
         while(_loc11_ < var_293.length)
         {
            _loc10_ = var_293[_loc11_] as Component;
            if(_loc10_ != this)
            {
      ...
      public final function detachComponent(param1:Component) : void
      {
         ...
         var _loc4_:int = 0;
         while(_loc4_ < var_293.length)
         {
            if(var_293[_loc4_] == param1)
            {
               var_293.splice(_loc4_,1);
               return;
            }
            _loc4_++;
         }
         ...
         while(i < componentList.length())
         {
            ...
            if(assetCount > 0)
            {
               ...
               while(k < assetCount)
               {
                  ...
                  {
                     error("Manifest for component " + componentString + " contains broken asset \"" + assetName + "\"!",true,Core.const_700);
                  }
                  k++;
               }
            }
            ...
            if(componentClass == null)
            {
               error("Invalid component class " + componentString + "!",true,Core.const_700);
               return false;
            }
            ...
            if(component != null)
            {
               if(assetLibrary != null)
               {
                  if(component.assets != assetLibrary)
                  {
                     ...
                     error("Component \"" + componentString + "\" did not save provided asset library!",true,Core.const_700);
                  }
               }
