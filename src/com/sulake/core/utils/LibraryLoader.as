package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1033:int = 5;
      ...
      protected var var_294:int;
      
      protected var var_2300:String = "";
      
      protected var var_165:int = 0;
      
      protected var var_481:XML;
      
      protected var var_1120:Boolean = true;
      
      protected var var_1399:Boolean = false;
      
      protected var var_459:uint = 0;
      ...
      protected var var_348:Class;
      
      protected var var_2409:uint = 0;
      ...
      {
         var_2299 = param1;
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,var_165,bytesTotal,bytesLoaded));
      }
      ...
      protected function prepareLibrary() : void
      {
         ...
         var_348 = this.getDefinition(_name) as Class;
         if(var_348 == null)
         {
         ...
         try
         {
            xmlClass = var_348.manifest as Class;
            if(xmlClass == null)
            {
               return;
            }
         ...
         var_481 = new XML(bytes.readUTFBytes(bytes.length));
      }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(var_1394)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,var_165,bytesTotal,bytesLoaded));
         }
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         switch(param1.type)
         {
            case Event.INIT:
               ...
               var_459 |= LibraryLoader.STATE_EVENT_INIT;
               break;
            case Event.COMPLETE:
               debug("Load event COMPLETE for file \"" + var_15.contentLoaderInfo.url + "\"");
               var_459 |= LibraryLoader.STATE_EVENT_COMPLETE;
               break;
            case Event.ENTER_FRAME:
               break;
            case HTTPStatusEvent.HTTP_STATUS:
               var_165 = HTTPStatusEvent(param1).status;
               debug("Load event STATUS " + var_165 + " for file \"" + var_15.contentLoaderInfo.url + "\"");
               break;
            case Event.UNLOAD:
               debug("Load event UNLOAD for file \"" + var_15.contentLoaderInfo.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,var_165,bytesTotal,bytesLoaded));
               break;
            case ProgressEvent.PROGRESS:
               debug("Load event PROGRESS for file \"" + var_15.contentLoaderInfo.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,var_165,bytesTotal,bytesLoaded));
               break;
         ...
         if(var_459 == LibraryLoader.STATE_ANALYZE)
         {
            if(analyzeLibrary())
            {
               var_459 |= LibraryLoader.STATE_EVENT_FRAME;
            }
         }
         if(var_459 == LibraryLoader.STATE_READY)
         {
            var_1399 = true;
            prepareLibrary();
            removeEventListeners();
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,var_165,bytesTotal,bytesLoaded));
         }
      }
